...
-- Контейнер для элементов управления
local ShiftLockControlContainer = Instance.new("Frame")
ShiftLockControlContainer.Size = UDim2.new(0, 150, 0, 25)
ShiftLockControlContainer.Position = UDim2.new(1, -110, 0.5, -12)
ShiftLockControlContainer.BackgroundTransparency = 1
ShiftLockControlContainer.Parent = ShiftLockContainer

-- Поле ввода для чувствительности
local SensitivityInput = Instance.new("TextBox")
SensitivityInput.Name = "SensitivityInput"
SensitivityInput.Size = UDim2.new(0, 40, 1, 0)
SensitivityInput.Position = UDim2.new(0, 0, 0, 0)
SensitivityInput.BackgroundColor3 = Color3.fromRGB(40, 40, 50)
SensitivityInput.TextColor3 = Color3.new(1, 1, 1)
SensitivityInput.Font = Enum.Font.Gotham
SensitivityInput.TextSize = 14
SensitivityInput.Text = "1.7"
SensitivityInput.Parent = ShiftLockControlContainer

-- Переключатель
local ShiftLockToggleFrame = Instance.new("Frame")
ShiftLockToggleFrame.Name = "ToggleFrame"
ShiftLockToggleFrame.Size = UDim2.new(0, 50, 0, 25)
ShiftLockToggleFrame.Position = UDim2.new(0, 50, 0, 0)
ShiftLockToggleFrame.BackgroundColor3 = Color3.fromRGB(50, 50, 60)
ShiftLockToggleFrame.Parent = ShiftLockControlContainer

local ShiftLockToggleCorner = Instance.new("UICorner")
ShiftLockToggleCorner.CornerRadius = UDim.new(1, 0)
ShiftLockToggleCorner.Parent = ShiftLockToggleFrame

local ShiftLockToggleButton = Instance.new("TextButton")
ShiftLockToggleButton.Name = "ToggleButton"
ShiftLockToggleButton.Size = UDim2.new(0, 21, 0, 21)
ShiftLockToggleButton.Position = UDim2.new(0, 2, 0.5, -10)
ShiftLockToggleButton.BackgroundColor3 = Color3.fromRGB(220, 220, 220)
ShiftLockToggleButton.Text = ""
ShiftLockToggleButton.Parent = ShiftLockToggleFrame

local ShiftLockButtonCorner = Instance.new("UICorner")
ShiftLockButtonCorner.CornerRadius = UDim.new(1, 0)
ShiftLockButtonCorner.Parent = ShiftLockToggleButton

-- Логика ShiftLock
local shiftLockEnabled = false
local shiftLockActive = false
local shiftLockConnection = nil
local shiftLockOffset = 1.7
local shiftLockCursor = nil
local shiftLockButton = nil

local function updateShiftLockToggle()
    local goal = {
        Position = shiftLockEnabled and UDim2.new(1, -23, 0.5, -10) or UDim2.new(0, 2, 0.5, -10),
        BackgroundColor3 = shiftLockEnabled and Color3.fromRGB(0, 230, 100) or Color3.fromRGB(220, 220, 220)
    }
    
    ShiftLockToggleFrame.BackgroundColor3 = shiftLockEnabled and Color3.fromRGB(0, 60, 30) or Color3.fromRGB(50, 50, 60)
    
    local tween = TweenService:Create(ShiftLockToggleButton, TweenInfo.new(0.2), goal)
    tween:Play()
end

local function createShiftLockUI()
    -- Создаем кнопку ShiftLock (уменьшенная версия)
    shiftLockButton = Instance.new("ImageButton")
    shiftLockButton.Name = "ShiftLockButton"
    shiftLockButton.Parent = ScreenGui
    shiftLockButton.BackgroundTransparency = 1
    shiftLockButton.Position = UDim2.new(0.95, 0, 0.02, 0)
    ...
    shiftLockCursor.Size = UDim2.new(0, 24, 0, 24)
    ...
    shiftLockButton.MouseButton1Click:Connect(function()
        ...
        shiftLockButton.Image = shiftLockActive and "rbxasset://textures/ui/mouseLock_on@2x.png" 
            or "rbxasset://textures/ui/mouseLock_off@2x.png"
        shiftLockCursor.Visible = shiftLockActive
    end)
end

local function enableShiftLock()
    shiftLockEnabled = true
    updateShiftLockToggle()
    ...
    shiftLockConnection = RunService.RenderStepped:Connect(function()
        if shiftLockEnabled and shiftLockActive and LocalPlayer.Character then
...
-- Обработчик переключателя
ShiftLockToggleButton.MouseButton1Click:Connect(function()
...
SensitivityInput.FocusLost:Connect(function()
    local num = tonumber(SensitivityInput.Text)
    if num and num >= 0.1 and num <= 5 then
        shiftLockOffset = num
    else
        SensitivityInput.Text = tostring(shiftLockOffset)
    end
end)
...
game:GetService("Players").PlayerRemoving:Connect(function(player)
    if player == LocalPlayer then
        disableShiftLock()
        if shiftLockButton then
            shiftLockButton:Destroy()
            shiftLockCursor:Destroy()
        end
    end
end)
