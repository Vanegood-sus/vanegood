...
local function createHitBoxHighlight(character, size)
    if HighlightInstances[character] then
        HighlightInstances[character]:Destroy()
    end
    
    local highlight = Instance.new("Highlight")
    highlight.Name = "HitBoxHighlight"
    highlight.FillTransparency = 1  -- Полностью прозрачная заливка
    highlight.OutlineColor = Color3.fromRGB(0, 255, 100)  -- Зеленая обводка
    highlight.OutlineTransparency = 0.2  -- Чуть прозрачная обводка
...
SizeInput.FocusLost:Connect(function()
    ...
    if num and num >= 1 and num <= 100 then
        _G.Size = num
        -- Обновляем размеры хитбоксов при изменении размера
        if _G.Disabled then
            for _, player in ipairs(Players:GetPlayers()) do
                if player ~= LocalPlayer and player.Character then
                    local rootPart = player.Character:FindFirstChild("HumanoidRootPart")
        ...
        for _, player in ipairs(Players:GetPlayers()) do
            if player ~= LocalPlayer and player.Character then
                pcall(function()
                    local rootPart = player.Character:FindFirstChild("HumanoidRootPart")
                    if rootPart then
                        -- Меняем размер корневой части
                        rootPart.Size = Vector3.new(_G.Size, _G.Size, _G.Size)
                        rootPart.Transparency = 1  -- Полностью прозрачный
                        rootPart.CanCollide = false
                        
                        -- Создаем или обновляем Highlight
                        if not HighlightInstances[player.Character] then
                            createHitBoxHighlight(player.Character, _G.Size)
                        end
                    end
                end)
            end
...
Players.PlayerRemoving:Connect(function(player)
    if player.Character then
        ...
        -- Удаляем Highlight при выходе игрока
        if HighlightInstances[player.Character] then
            HighlightInstances[player.Character]:Destroy()
            HighlightInstances[player.Character] = nil
        end
    end
end)
...
Players.PlayerAdded:Connect(function(player)
    player.CharacterAdded:Connect(function(character)
        ...
        if _G.Disabled and player ~= LocalPlayer then
            pcall(function()
                ...
                if rootPart then
                    ...
                    rootPart.CanCollide = false
                    createHitBoxHighlight(character, _G.Size)
                end
            end)
        end
    end)
