...
local function createDanceToggle(container)
    ...
    -- Функция для получения случайного танца (без повторения предыдущего)
    local function getRandomDance()
        ...
        for _, danceId in ipairs(dances) do
            if danceId ~= lastDanceId then
                table.insert(availableDances, danceId)
            end
        end
...
-- Добавление в ваше меню (в раздел скриптов)
