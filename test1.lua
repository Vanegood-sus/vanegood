...
-- Teleport with Loop Effect
local TeleportContainer = Instance.new("Frame")
TeleportContainer.Name = "TeleportSettings"
TeleportContainer.Size = UDim2.new(1, -20, 0, 40)
TeleportContainer.Position = UDim2.new(0, 10, 0, 60) -- Позиция после WalkFling
TeleportContainer.BackgroundColor3 = Color3.fromRGB(30, 30, 40)
TeleportContainer.BackgroundTransparency = 0.5
TeleportContainer.Parent = TrollFrame
...
TeleportLabel.Position = UDim2.new(0, 10, 0, 0)
TeleportLabel.BackgroundTransparency = 1
TeleportLabel.Text = "Be in the player"
TeleportLabel.TextColor3 = Color3.fromRGB(220, 220, 220)
TeleportLabel.Font = Enum.Font.GothamBold
TeleportLabel.TextSize = 14
TeleportLabel.TextXAlignment = Enum.TextXAlignment.Left
...
TeleportControlContainer.Size = UDim2.new(0, 180, 0, 25)
TeleportControlContainer.Position = UDim2.new(1, -190, 0.5, -12)
...
TeleportToggleButton.Name = "TeleportToggle"
TeleportToggleButton.Size = UDim2.new(0, 120, 0, 25)
TeleportToggleButton.Position = UDim2.new(0, 0, 0, 0)
TeleportToggleButton.BackgroundColor3 = Color3.fromRGB(50, 50, 60)
TeleportToggleButton.Text = "Select ▷"
TeleportToggleButton.TextColor3 = Color3.fromRGB(220, 220, 220)
TeleportToggleButton.Font = Enum.Font.Gotham
TeleportToggleButton.TextSize = 12
TeleportToggleButton.Parent = TeleportControlContainer
...
LoopToggleButton.Name = "LoopToggle"
LoopToggleButton.Size = UDim2.new(0, 50, 0, 25)
LoopToggleButton.Position = UDim2.new(0, 130, 0, 0)
LoopToggleButton.BackgroundColor3 = Color3.fromRGB(50, 50, 60)
LoopToggleButton.Text = "Loop"
LoopToggleButton.TextColor3 = Color3.fromRGB(220, 220, 220)
LoopToggleButton.Font = Enum.Font.Gotham
LoopToggleButton.TextSize = 12
LoopToggleButton.Parent = TeleportControlContainer

local LoopButtonCorner = Instance.new("UICorner")
LoopButtonCorner.CornerRadius = UDim.new(0, 4)
...
-- Переменные для Loop Mode
local loopTeleportEnabled = false
local loopTarget = nil
local loopConnection = nil
local loopInterval = 0.5 -- Интервал между телепортациями в секундах

local function teleportToPlayer(player)
    if player and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
        local char = LocalPlayer.Character
        if char and char:FindFirstChild("HumanoidRootPart") then
            -- Добавляем эффект "зацикливания" с небольшим смещением
            local targetCFrame = player.Character.HumanoidRootPart.CFrame
            local offset = CFrame.new(math.random(-2, 2), 0, math.random(-2, 2))
            char.HumanoidRootPart.CFrame = targetCFrame * offset
            
            -- Эффект частиц для визуализации телепорта
            local teleportEffect = Instance.new("ParticleEmitter")
            teleportEffect.LightEmission = 1
            teleportEffect.Texture = "rbxassetid://242487985"
            teleportEffect.Color = ColorSequence.new(Color3.fromRGB(0, 170, 255))
            teleportEffect.Size = NumberSequence.new(0.5)
            teleportEffect.Lifetime = NumberRange.new(0.5)
            teleportEffect.Speed = NumberRange.new(2)
            teleportEffect.EmissionDirection = Enum.NormalId.Top
            teleportEffect.ZOffset = 1
            teleportEffect.Parent = char.HumanoidRootPart
...
local function startLoopTeleport(player)
    ...
    loopTarget = player
    loopTeleportEnabled = true
    LoopToggleButton.BackgroundColor3 = Color3.fromRGB(0, 170, 255)
    
    loopConnection = game:GetService("RunService").Heartbeat:Connect(function()
        if loopTeleportEnabled and loopTarget then
            teleportToPlayer(loopTarget)
            wait(loopInterval)
        end
    end)
end
...
LoopToggleButton.MouseButton1Click:Connect(function()
    if loopTeleportEnabled then
        ...
        if loopTarget then
            startLoopTeleport(loopTarget)
        else
            -- Если цель не выбрана, открываем меню выбора
            isMenuOpen = true
            TeleportToggleButton.Text = "Select ◁"
            updateMenuPosition()
            PlayersSideMenu.Visible = true
...
HeadsitContainer.Position = UDim2.new(0, 10, 0, 110) -- Позиция после Teleport
...
HeadsitControlContainer.Size = UDim2.new(0, 180, 0, 25)
HeadsitControlContainer.Position = UDim2.new(1, -190, 0.5, -12)
...
HeadsitToggleButton.Position = UDim2.new(0, 0, 0, 0)
...
HeadsitToggleButton.Parent = HeadsitControlContainer
...
-- Кнопка активации
local ActivateHeadsitButton = Instance.new("TextButton")
ActivateHeadsitButton.Name = "ActivateHeadsit"
ActivateHeadsitButton.Size = UDim2.new(0, 50, 0, 25)
ActivateHeadsitButton.Position = UDim2.new(0, 130, 0, 0)
ActivateHeadsitButton.BackgroundColor3 = Color3.fromRGB(50, 50, 60)
ActivateHeadsitButton.Text = "Sit"
ActivateHeadsitButton.TextColor3 = Color3.fromRGB(220, 220, 220)
ActivateHeadsitButton.Font = Enum.Font.Gotham
ActivateHeadsitButton.TextSize = 12
ActivateHeadsitButton.Parent = HeadsitControlContainer

local ActivateButtonCorner = Instance.new("UICorner")
ActivateButtonCorner.CornerRadius = UDim.new(0, 4)
ActivateButtonCorner.Parent = ActivateHeadsitButton

-- Меню выбора игроков (используем то же самое, что и для телепорта)
local HeadsitPlayersMenu = PlayersSideMenu:Clone()
HeadsitPlayersMenu.Name = "HeadsitPlayersMenu"
HeadsitPlayersMenu.Parent = ScreenGui

-- Переменные для Headsit
local headsitEnabled = false
local headsitTarget = nil
local headsitConnection = nil
local headsitOriginalSeat = nil
...
local function startHeadsit(player)
    ...
    headsitTarget = player
    headsitEnabled = true
    ActivateHeadsitButton.BackgroundColor3 = Color3.fromRGB(0, 170, 255)
    ActivateHeadsitButton.Text = "Sitting"
    
    -- Заставляем персонажа сесть
    humanoid.Sit = true
    
    -- Сохраняем оригинальное сиденье (если есть)
    if humanoid.SeatPart then
        headsitOriginalSeat = humanoid.SeatPart
    end
    
    headsitConnection = RunService.Heartbeat:Connect(function()
        if headsitEnabled and headsitTarget and headsitTarget.Character and character and humanoid then
            local targetRoot = getRoot(headsitTarget.Character)
            local myRoot = getRoot(character)
            
            if targetRoot and myRoot and humanoid.Sit then
                -- Сидим на голове с небольшим смещением вперед для лучшего вида
                myRoot.CFrame = targetRoot.CFrame * CFrame.Angles(0, math.rad(0), 0) * CFrame.new(0, 1.6, 0.4)
                
                -- Добавляем эффект "покачивания" для веселья
                local waveOffset = math.sin(os.clock() * 5) * 0.05
                myRoot.CFrame = myRoot.CFrame * CFrame.new(0, waveOffset, 0)
            else
                stopHeadsit()
            end
        else
...
local function stopHeadsit()
    ...
    if character then
        ...
        if humanoid then
            ...
            -- Восстанавливаем оригинальное сиденье (если было)
            if headsitOriginalSeat and headsitOriginalSeat.Parent then
                humanoid.Sit = true
                humanoid.SeatPart = headsitOriginalSeat
            end
            headsitOriginalSeat = nil
        end
    end
end
...
local function createHeadsitPlayerButton(player)
    ...
    button.Text = player.Name
    button.TextXAlignment = Enum.TextXAlignment.Left
    button.Parent = HeadsitPlayersMenu:FindFirstChild("ScrollingFrame")
    ...
    button.MouseButton1Click:Connect(function()
        ...
        -- Автоматически активируем сидение при выборе игрока
...
ActivateHeadsitButton.MouseButton1Click:Connect(function()
    if headsitEnabled then
        stopHeadsit()
    else
...
Players.PlayerRemoving:Connect(function(player)
    for _, child in ipairs(HeadsitPlayersMenu:FindFirstChild("ScrollingFrame"):GetChildren()) do
        if child:IsA("TextButton") and child.Text == player.Name then
            child:Destroy()
            if headsitTarget == player then
                stopHeadsit()
                headsitTarget = nil
            end
            break
...
-- Обработчик изменения позиции контейнера
HeadsitContainer:GetPropertyChangedSignal("AbsolutePosition"):Connect(function()
    if isHeadsitMenuOpen then
        HeadsitPlayersMenu.Position = UDim2.new(
            0, HeadsitContainer.AbsolutePosition.X + HeadsitContainer.AbsoluteSize.X + 5,
            0, HeadsitContainer.AbsolutePosition.Y - 135
