...
InsideContainer.Name = "InsideSettings"
InsideContainer.Size = UDim2.new(1, -20, 0, 40)
InsideContainer.Position = UDim2.new(0, 10, 0, 60) -- Позиция после WalkFling
InsideContainer.BackgroundColor3 = Color3.fromRGB(30, 30, 40)
InsideContainer.BackgroundTransparency = 0.5
InsideContainer.Parent = TrollFrame

local InsideCorner = Instance.new("UICorner")
InsideCorner.CornerRadius = UDim.new(0, 6)
InsideCorner.Parent = InsideContainer

local InsideLabel = Instance.new("TextLabel")
InsideLabel.Name = "Label"
InsideLabel.Size = UDim2.new(0, 120, 1, 0)
InsideLabel.Position = UDim2.new(0, 10, 0, 0)
InsideLabel.BackgroundTransparency = 1
InsideLabel.Text = "Be Inside Player"
InsideLabel.TextColor3 = Color3.fromRGB(220, 220, 220)
InsideLabel.Font = Enum.Font.GothamBold
InsideLabel.TextSize = 14
InsideLabel.TextXAlignment = Enum.TextXAlignment.Left
InsideLabel.Parent = InsideContainer

local InsideControlContainer = Instance.new("Frame")
InsideControlContainer.Size = UDim2.new(0, 150, 0, 25)
InsideControlContainer.Position = UDim2.new(1, -110, 0.5, -12)
InsideControlContainer.BackgroundTransparency = 1
InsideControlContainer.Parent = InsideContainer

-- Поле ввода для смещения
local InsideOffsetInput = Instance.new("TextBox")
InsideOffsetInput.Name = "OffsetInput"
InsideOffsetInput.Size = UDim2.new(0, 40, 1, 0)
InsideOffsetInput.Position = UDim2.new(0, 0, 0, 0)
InsideOffsetInput.BackgroundColor3 = Color3.fromRGB(40, 40, 50)
InsideOffsetInput.TextColor3 = Color3.new(1, 1, 1)
InsideOffsetInput.Font = Enum.Font.Gotham
InsideOffsetInput.TextSize = 14
InsideOffsetInput.Text = "0"
InsideOffsetInput.Parent = InsideControlContainer

-- Переключатель
local InsideToggleFrame = Instance.new("Frame")
InsideToggleFrame.Name = "ToggleFrame"
InsideToggleFrame.Size = UDim2.new(0, 50, 0, 25)
InsideToggleFrame.Position = UDim2.new(0, 50, 0, 0)
InsideToggleFrame.BackgroundColor3 = Color3.fromRGB(50, 50, 60)
InsideToggleFrame.Parent = InsideControlContainer

local InsideToggleCorner = Instance.new("UICorner")
InsideToggleCorner.CornerRadius = UDim.new(1, 0)
InsideToggleCorner.Parent = InsideToggleFrame

local InsideToggleButton = Instance.new("TextButton")
InsideToggleButton.Name = "ToggleButton"
InsideToggleButton.Size = UDim2.new(0, 21, 0, 21)
InsideToggleButton.Position = UDim2.new(0, 2, 0.5, -10)
InsideToggleButton.BackgroundColor3 = Color3.fromRGB(220, 220, 220)
InsideToggleButton.Text = ""
InsideToggleButton.Parent = InsideToggleFrame

local InsideButtonCorner = Instance.new("UICorner")
InsideButtonCorner.CornerRadius = UDim.new(1, 0)
InsideButtonCorner.Parent = InsideToggleButton

-- Логика Inside Player
local insideEnabled = false
local insideOffset = 0
local insideConnections = {}
local insideTarget = nil
local insideNoClip = false

local function getRoot(character)
    return character:FindFirstChild("HumanoidRootPart") or character:FindFirstChild("Torso") or character:FindFirstChild("UpperTorso")
end

local function updateInsideToggle()
    local goal = {
        Position = insideEnabled and UDim2.new(1, -23, 0.5, -10) or UDim2.new(0, 2, 0.5, -10),
        BackgroundColor3 = insideEnabled and Color3.fromRGB(0, 230, 100) or Color3.fromRGB(220, 220, 220)
    }
    
    InsideToggleFrame.BackgroundColor3 = insideEnabled and Color3.fromRGB(0, 60, 30) or Color3.fromRGB(50, 50, 60)
    
    local tween = TweenService:Create(InsideToggleButton, TweenInfo.new(0.2), goal)
    tween:Play()
end

local function enableInsideNoClip()
    insideNoClip = true
    local character = LocalPlayer.Character
    if not character then return end
    ...
    table.insert(insideConnections, character.DescendantAdded:Connect(function(part)
        if part:IsA("BasePart") then
            part.CanCollide = false
        end
    end))
...
local function disableInsideNoClip()
    insideNoClip = false
    local character = LocalPlayer.Character
    if not character then return end
...
local function teleportInsidePlayer(player)
    ...
    if targetRoot and myRoot then
        -- Получаем CFrame цели и добавляем смещение
        local targetCFrame = targetRoot.CFrame
        myRoot.CFrame = targetCFrame * CFrame.new(0, insideOffset, 0)
...
local function enableInsidePlayer()
    if not insideTarget then return end
    ...
    -- Подключение для обработки смерти персонажа
    table.insert(insideConnections, LocalPlayer.CharacterAdded:Connect(function(character)
        character:WaitForChild("Humanoid").Died:Connect(function()
            disableInsidePlayer()
        end)
    end))
    ...
    -- Основной цикл
    table.insert(insideConnections, RunService.Heartbeat:Connect(function()
        if not insideEnabled then return end
        teleportInsidePlayer(insideTarget)
    end))
end

local function disableInsidePlayer()
    insideEnabled = false
    updateInsideToggle()
    disableInsideNoClip()
    ...
    for _, connection in ipairs(insideConnections) do
        connection:Disconnect()
    end
    insideConnections = {}
end

InsideToggleButton.MouseButton1Click:Connect(function()
    if insideEnabled then
        disableInsidePlayer()
    else
...
InsideOffsetInput.FocusLost:Connect(function()
    local num = tonumber(InsideOffsetInput.Text)
    if num then
        insideOffset = num
    else
        InsideOffsetInput.Text = tostring(insideOffset)
    end
end)
