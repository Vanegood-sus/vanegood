...
local function enableFly()
    ...
    local function setupFly(character)
        if not character or not _G.FlyEnabled then return end
        
        local humanoid = character:WaitForChildOfClass("Humanoid")
        local rootPart = character:WaitForChild("HumanoidRootPart")
        
        -- Очищаем старые BodyVelocity/BodyGyro
        for _, v in pairs(rootPart:GetChildren()) do
            if v:IsA("BodyVelocity") or v:IsA("BodyGyro") then
                v:Destroy()
            end
    ...
    -- Обработчик для текущего персонажа
    if LocalPlayer.Character then
    ...
    table.insert(flyConnections, RunService.Heartbeat:Connect(function()
        ...
        local bv = rootPart and rootPart:FindFirstChild("FlyVelocity")
        local bg = rootPart and rootPart:FindFirstChild("FlyGyro")
        
        if not humanoid or not rootPart or not bv or not bg then
