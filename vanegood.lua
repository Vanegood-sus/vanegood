...
-- Безопасно получаем текущие возрождения
    ...
    local toggle = farmGymsFolder:AddSwitch(spanishWorkoutName, function(bool)
        ...
        if bool then
            ...
            if workoutPositions[workoutType] and workoutPositions[workoutType][selectedGym] then
                ...
                -- Start the workout - Teleport to gym
                local targetPosition = workoutPositions[workoutType][selectedGym]
                if game.Players.LocalPlayer.Character and game.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = targetPosition
                    
                    game:GetService("StarterGui"):SetCore("SendNotification", {
                        Title = "Телепорт",
                        Text = "Телепортирован в " .. selectedGym .. " для " .. workoutType,
                        Duration = 3
                    })
                end
            else
...
noteTab:AddLabel("Переведено на русский язык")
noteTab:AddLabel("")
noteTab:AddLabel("Удачи!")

-- Final notification that script loaded successfully
game:GetService("StarterGui"):SetCore("SendNotification", {
    Title = "VaneGood Script",
