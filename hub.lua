...
-- Загрузка внешних файлов (scripts.lua, games.lua, trolling.lua) — безопасный pcall
local function loadExternalScript(url, flagName)
...
local function loadScripts()
    loadExternalScript("https://raw.githubusercontent.com/Vanegood-sus/vanegood/main/scripts.lua", "Vanegood_ScriptsLoaded")
end

local function loadGames()
    loadExternalScript("https://raw.githubusercontent.com/Vanegood-sus/vanegood/main/games.lua", "Vanegood_GamesLoaded")
end

local function loadTrolling()
    loadExternalScript("https://raw.githubusercontent.com/Vanegood-sus/vanegood/main/trolling.lua", "Vanegood_TrollingLoaded")
end

-- Обработчики вкладок (подключаем загрузку при переключении на соответствующую вкладку)
...
-- Инициализация: показываем скрипты и сразу пробуем подгрузить scripts.lua
switchTab("scripts")
loadScripts()

-- (Опционально) если хочешь, чтобы hub также автоматически подгружал игры и троллинг на старте, раскомментируй:
-- loadGames()
-- loadTrolling()
